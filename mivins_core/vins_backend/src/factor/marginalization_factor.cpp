...
void MarginalizationInfo::addResidualBlockInfo(ResidualBlockInfo *residual_block_info)
{
  ...
  {
    int drop_idx = residual_block_info->drop_set[i];
    long addr = reinterpret_cast<long>(parameter_blocks[drop_idx]);
...
void* ThreadsConstructA(void* threadsstruct)
{
  ...
  for (auto it : p->sub_factors)
  {
    for (int i = 0; i < static_cast<int>(it->parameter_blocks.size()); i++)
    {
      ...
      int idx_i = p->parameter_block_idx[addr_i];
      int size_i = p->parameter_block_size[addr_i];
      ...
      for (int j = i; j < static_cast<int>(it->parameter_blocks.size()); j++)
      {
        ...
        int idx_j = p->parameter_block_idx[addr_j];
        int size_j = p->parameter_block_size[addr_j];
        if (size_j == 7) size_j = 6;

        Eigen::MatrixXd jacobian_j = it->jacobians[j].leftCols(size_j);
        Eigen::MatrixXd Hxx = jacobian_i.transpose() * jacobian_j; 

        p->A.block(idx_i, idx_j, size_i, size_j) += Hxx;
...
Eigen::MatrixXd MarginalizationInfo::pseudoInverse(const Eigen::MatrixXd &Amm)
{
  Eigen::MatrixXd Amm_inv;

  // Eigen::SelfAdjointEigenSolver<Eigen::MatrixXd> saes(Amm);
  // Amm_inv = saes.eigenvectors() * Eigen::VectorXd((saes.eigenvalues().array() > eps).select(
  //       saes.eigenvalues().array().inverse(), 0)).asDiagonal() * saes.eigenvectors().transpose();
  
  auto Amm_decomposition = Amm.completeOrthogonalDecomposition();
  Amm_inv = Amm_decomposition.pseudoInverse();

  // auto Amm_decomposition = Amm.fullPivLu();
  // Amm_inv = Amm_decomposition.inverse();

  // auto Amm_decomposition = Amm.colPivHouseholderQr();
  // Amm_inv = Amm_decomposition.inverse();
...
void MarginalizationInfo::marginalize()
{
  ...
  // auto Amm_decomposition = Amm.completeOrthogonalDecomposition();
  // Eigen::MatrixXd Amm_inv = Amm_decomposition.pseudoInverse();
