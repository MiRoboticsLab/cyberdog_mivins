...
double DiffBundleSameCamFactor::sum_t_;

DiffBundleSameCamFactor::DiffBundleSameCamFactor(
...
void DiffBundleSameCamFactor::setInitExtrinsicParam(
...
bool DiffBundleSameCamFactor::Evaluate(
        ...
        double *residuals, double **jacobians) const
{
  ...
  setExtPose(parameters[2], 1);

  setInvDepth(parameters[3]);
  ...
  if (jacobians)
  {
    ...
    if (jacobians[0])
    {
      ...
      jaco_i.leftCols<3>() = ric_i.transpose() * Rj.transpose();
      jaco_i.rightCols<3>() = ric_i.transpose() * Rj.transpose() * Ri * -Utility::skewSymmetric(pts_imu_i);
    ...
    if (jacobians[1])
    {
      ...
      jaco_j.leftCols<3>() = ric_i.transpose() * -Rj.transpose();
      jaco_j.rightCols<3>() = ric_i.transpose() * Utility::skewSymmetric(pts_imu_j);
    ...
    if (jacobians[2])
    {
      ...
      jaco_ex.leftCols<3>() = ric_i.transpose() * (Rj.transpose() * Ri - Eigen::Matrix3d::Identity());
      Eigen::Matrix3d tmp_r = ric_i.transpose() * Rj.transpose() * Ri * ric_i;
      jaco_ex.rightCols<3>() = -tmp_r * Utility::skewSymmetric(pts_cam_i) + Utility::skewSymmetric(tmp_r * pts_cam_i) +
                   Utility::skewSymmetric(ric_i.transpose() * (Rj.transpose() * (Ri * tic_i_ + Pi_ - Pj_) - tic_i_));
    ...
    if (jacobians[3])
    {
      ...
      jacobian_feature = reduce * ric_i.transpose() * Rj.transpose() * Ri * ric_i * pts_i_td_ * -(1.0 / (inv_dep_i_ * inv_dep_i_));
    }
    if (jacobians[4])
    {
      Eigen::Map<Eigen::Vector2d> jacobian_td(jacobians[4]);
      jacobian_td = reduce * ric_i.transpose() * Rj.transpose() * Ri * ric_i * vel_i_ / inv_dep_i_ * -1.0  +
...
void DiffBundleSameCamFactor::check(double **parameters)
{
  ...
  Eigen::Vector3d pts_cam_j = qic_i.inverse() * (pts_imu_j - tic_i);
...
DiffBundleSameCamFactorP::DiffBundleSameCamFactorP(
  const Eigen::Vector3d &pts_i, const Eigen::Vector3d &pts_j, 
  const int pyr_i, const int pyr_j)
{  
  pyr_i_ = pyr_i;
...
void DiffBundleSameCamFactorP::setInitExtrinsicParam(
    const Eigen::Matrix3d &ric_i, const Eigen::Vector3d &tic_i,
    const Eigen::Matrix3d &ric_j, const Eigen::Vector3d &tic_j)
{
  qic_i_ = ric_i;
...
bool DiffBundleSameCamFactorP::Evaluate(
        double const *const *parameters, 
        double *residuals, double **jacobians) const
{
  setImuPose(parameters[0], 0);
  ...
  if (jacobians)
  {
    ...
    Eigen::Matrix3d R_cj_w_tmp = ric_i.transpose() * Rj.transpose();
    ...
    if (jacobians[1])
    {
      ...
      jacobian_pose_j.block(0, 3, 2, 3) = reduce * ric_i.transpose() * Utility::skewSymmetric(pts_imu_j);
    }
    if (jacobians[2])
    {
