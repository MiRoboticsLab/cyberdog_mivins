...
/*
  for (int i = 0; i < frame_num; i++)
  {
    ...
    std::cout << "solvePnP  q" << " i " << i <<"  " <<q[i].w() << "  " << q[i].vec().transpose() << std::endl;
  }
  for (int i = 0; i < frame_num; i++)
  {
    Vector3d t_tmp;
    t_tmp = -1 * (q[i] * c_Translation[i]);
    std::cout << "solvePnP  t" << " i " << i <<"  " << t_tmp.x() <<"  "<< t_tmp.y() <<"  "<< t_tmp.z() << std::endl;
  }
*/
  //full BA
  ...
  if (summary.termination_type == ceres::CONVERGENCE || summary.final_cost < 5) //5e-03)
  {
