...
namespace mivins
{
    ...
    public:
        ...
        OccupandyGrid2D(int cell_size, int n_cols, int n_rows)
            : cell_size(cell_size), n_cols(n_cols), n_rows(n_rows), occupancy_(n_cols * n_rows, false), feature_occupancy_(n_cols * n_rows, Keypoint(0, 0))
        {
        }

        OccupandyGrid2D(const OccupandyGrid2D &rhs)
            : cell_size(rhs.cell_size), n_cols(rhs.n_cols), n_rows(rhs.n_rows), occupancy_(rhs.occupancy_), feature_occupancy_(rhs.feature_occupancy_)
        {
        }
        ...
        {
            return std::ceil(static_cast<double>(n_pixels) / static_cast<double>(size));
        }
