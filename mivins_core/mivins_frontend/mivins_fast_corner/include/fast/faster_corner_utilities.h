...
namespace fast
{
    ...
    template <>
    inline bool IsAligned<8>(const void *ptr) { return ((reinterpret_cast<std::size_t>(ptr)) & 0x7) == 0; }
    template <>
    inline bool IsAligned<16>(const void *ptr) { return ((reinterpret_cast<std::size_t>(ptr)) & 0xF) == 0; }
...
#define CHECK_BARRIER(lo, hi, other, flags)                                         \
    ...
    inline uint8x16_t load_ui128(const void *addr)
    {
...
#define CHECK_BARRIER(lo, hi, other, flags)                                    \
    ...
    template <>
    inline __m128i load_si128<true>(const void *addr) { return _mm_load_si128((const __m128i *)addr); }
