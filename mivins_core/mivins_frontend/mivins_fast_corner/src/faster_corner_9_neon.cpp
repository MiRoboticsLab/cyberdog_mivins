...
namespace fast
{
                           ...
                           short barrier, std::vector<Fastxy> &corners)
    {
        ...
        for (int y = 3; y < imgHeight - 3; ++y)
        {
            for (int x = 3; x < 16; ++x)
            {
                if (Corner9<Less>((const unsigned char *)img + y * widthStep + x, widthStep, barrier) ||
                    Corner9<Greater>((const unsigned char *)img + y * widthStep + x, widthStep, barrier))
    ...
    void FastCornerDetect9Neon(const fast_byte *img, int imgWidth, int imgHeight, int widthStep,
                               short barrier, std::vector<Fastxy> &corners)
    {
        ...
        {
            FastCornerDetect9(img, imgWidth, imgHeight, widthStep, barrier, corners);
            return;
        }
        else
        {
            ...
            // corners.reserve(512);
            // if (IsAligned<16>(img) && IsAligned<16>(img + widthStep))
            //    FastCornerDetect9<true>(img, imgWidth, imgHeight, widthStep, barrier, corners);
            // else
            FastCornerDetect9<false>(img, imgWidth, imgHeight, widthStep, barrier, corners);
        }
    }
