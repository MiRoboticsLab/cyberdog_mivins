...
namespace fast
{
    ...
    void FasterCornerDetect10(const fast_byte *img, int img_width, int img_height, int img_stride,
                              short barrier, std::vector<Fastxy> &corners)
    {
        ...
        {
            for (int x = 3; x < 16; x++)
                if (Corner10<Less>((fast_byte *)img + y * img_stride + x, img_stride, barrier) ||
                    Corner10<Greater>((fast_byte *)img + y * img_stride + x, img_stride, barrier))
                    corners.push_back(Fastxy(x, y));
            ...
            for (int x = 16; x < xend; x += 16)
            {
                ...
                {
                    if (possible & (1 << 0))
                        corners.push_back(Fastxy(x + 0, y));
                    if (possible & (1 << 1))
                        corners.push_back(Fastxy(x + 1, y));
                    if (possible & (1 << 2))
                        corners.push_back(Fastxy(x + 2, y));
                    if (possible & (1 << 3))
                        corners.push_back(Fastxy(x + 3, y));
                    if (possible & (1 << 4))
                        corners.push_back(Fastxy(x + 4, y));
                    if (possible & (1 << 5))
                        corners.push_back(Fastxy(x + 5, y));
                    if (possible & (1 << 6))
                        corners.push_back(Fastxy(x + 6, y));
                    if (possible & (1 << 7))
                        corners.push_back(Fastxy(x + 7, y));
                }
                //if(possible & 0xf0) //Does this mak( ,  fast)r?
                {
                    if (possible & (1 << 8))
                        corners.push_back(Fastxy(x + 8, y));
                    if (possible & (1 << 9))
                        corners.push_back(Fastxy(x + 9, y));
                    if (possible & (1 << 10))
                        corners.push_back(Fastxy(x + 10, y));
                    if (possible & (1 << 11))
                        corners.push_back(Fastxy(x + 11, y));
                    if (possible & (1 << 12))
                        corners.push_back(Fastxy(x + 12, y));
                    if (possible & (1 << 13))
                        corners.push_back(Fastxy(x + 13, y));
                    if (possible & (1 << 14))
                        corners.push_back(Fastxy(x + 14, y));
                    if (possible & (1 << 15))
                        corners.push_back(Fastxy(x + 15, y));
                }
            }

            for (int x = xend; x < img_width - 3; x++)
                if (Corner10<Less>(img + y * img_stride + x, img_stride, barrier) ||
                    Corner10<Greater>(img + y * img_stride + x, img_stride, barrier))
                    corners.push_back(Fastxy(x, y));
        }
    }

    void FastCornerDetect10Sse2(const fast_byte *img, int img_width, int img_height, int img_stride,
                                short barrier, std::vector<Fastxy> &corners)
    {
        if (img_width < 22)
        {
            FastCornerDetect10(img, img_width, img_height, img_stride, barrier, corners);
            return;
        }
        else if (img_width < 22 || img_height < 7)
            return;

        if (IsAligned<16>(img) && IsAligned<16>(img + img_stride))
            FasterCornerDetect10<true>(img, img_width, img_height, img_stride, barrier, corners);
        else
            FasterCornerDetect10<false>(img, img_width, img_height, img_stride, barrier, corners);
    }
}
