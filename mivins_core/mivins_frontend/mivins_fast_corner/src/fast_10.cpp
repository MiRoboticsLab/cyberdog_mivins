...
namespace fast
{
    ...
    void FastCornerDetect10(const fast_byte *img, int img_width, int img_height, int img_stride,
                            short barrier, std::vector<Fastxy> &corners)
    {
        ...
        for (y = 3; y < img_height - 3; y++)
        {
            ...
            success:
                corners.push_back(Fastxy(static_cast<short>(cache_0 - line_min), static_cast<short>(y)));
            }
        }
    }
