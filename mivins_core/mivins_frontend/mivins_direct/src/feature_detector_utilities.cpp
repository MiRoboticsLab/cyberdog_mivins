...
namespace mivins
{
    namespace feature_detector_utils
    {
                          ...
                          OccupandyGrid2D &grid)
        {
            ...
            mivins::Aligned<std::vector, Keypoint> keypoint_vec;
            mivins::Aligned<std::vector, GradientVector> gradient_vec;
            ...
            OccupandyGrid2D &grid)
        {
            ...
            for (size_t level = min_level; level <= max_level; ++level)
            {
                ...
                std::vector<fast::Fastxy> fast_corners;
#if __SSE2__
                //std::cout << "-------use_fast_sse-------" << std::endl;
                fast::FastCornerDetect10Sse2(
                    (fast::fast_byte *)img_pyr[level].data, img_pyr[level].cols,
                    img_pyr[level].rows, img_pyr[level].step, threshold, fast_corners);
#elif HAVE_FAST_NEON
                //std::cout << "-------use_fast_neon-------" << std::endl;
                fast::FastCornerDetect9Neon(
                    (fast::fast_byte *)img_pyr[level].data, img_pyr[level].cols,
                    img_pyr[level].rows, img_pyr[level].step, threshold, fast_corners);
#else
                //std::cout << "-------fast-------" << std::endl;
                fast::FastCornerDetect10(
                    (fast::fast_byte *)img_pyr[level].data, img_pyr[level].cols,
                    img_pyr[level].rows, img_pyr[level].step, threshold, fast_corners);
#endif
                std::vector<int> scores, nm_corners;
                fast::FastCornerScore10((fast::fast_byte *)img_pyr[level].data, img_pyr[level].step,
                                        fast_corners, threshold, scores);
                fast::FastNonmax3x3(fast_corners, scores, nm_corners);

                const int maxw = img_pyr[level].cols - border;
                const int maxh = img_pyr[level].rows - border;
                for (const int &i : nm_corners)
                {
                    fast::Fastxy &xy = fast_corners.at(i);
            ...
            OccupandyGrid2D &grid)
        {
            ...
            //cv::Mat angle(img_pyr[level].size(), CV_8UC1, cv::Scalar(255));//no use
            ...
            double *angle)
        {
            ...
            if (px(0) < kHalfPatchSizePlusBorder || px(1) < kHalfPatchSizePlusBorder || px(0) >= img.cols - kHalfPatchSizePlusBorder || px(1) >= img.rows - kHalfPatchSizePlusBorder)
            {
            ...
            double *score)
        {
            ...
            const int x_min = px(0) - kHalfPatchSize; //col
            const int x_max = px(0) + kHalfPatchSize; //col
            const int y_min = px(1) - kHalfPatchSize; //row
            const int y_max = px(1) + kHalfPatchSize; //row
        ...
        //------------------------------------------------------------------------------
        void SetCornerAngles(const ImgPyramid &img_pyr, Corners *corners)
        {
        ...
        //------------------------------------------------------------------------------
        void setCornerLevel(const ImgPyramid &mag_pyr, Corners *corners)
        {
            ...
            const cv::Mat &img, const Eigen::Vector2i &px, const size_t halfpatch_size)
        {
            ...
            hist.fill(0.0);
            angle_hist::GetAngleHistogram(img, px(0), px(1), halfpatch_size, hist);
        ...
        Corners DistributeOctTree(
            const Corners& extracted_corners, const int &min_x,
            const int &max_x, const int &min_y, const int &max_y, const int &N)
        {
            const int init_cnt = round(static_cast<float>(max_x - min_x) / (max_y - min_y));
            const float size_x = static_cast<float>(max_x - min_x) / init_cnt;

            std::list<ExtractorNode> nodes_list;
            std::vector<ExtractorNode*> nodes_vec;
            nodes_vec.resize(init_cnt);

            for(int i = 0; i < init_cnt; ++i)
            {
                ExtractorNode node;
                node.UL = cv::Point2i(size_x * static_cast<float>(i), 0);
                node.UR = cv::Point2i(size_x * static_cast<float>(i + 1), 0);
                node.BL = cv::Point2i(node.UL.x, max_y - min_y);
                node.BR = cv::Point2i(node.UR.x, max_y - min_y);
                node.corners.reserve(extracted_corners.size());

                nodes_list.push_back(node);
                nodes_vec[i] = &nodes_list.back();
            }

            for(size_t i = 0; i < extracted_corners.size(); ++i)
            {
                const Corner &corner = extracted_corners[i];
                nodes_vec[corner.x / size_x]->corners.push_back(corner);
            }

            std::list<ExtractorNode>::iterator lit = nodes_list.begin();  
            while(lit != nodes_list.end())
            {
                if(lit->corners.size() == 1)
                {
                    lit->no_more = true;
                    lit++;
                }
                else if(lit->corners.empty())
                    lit = nodes_list.erase(lit);
                else
                    lit++;
            }
            
            std::vector<std::pair<int, ExtractorNode*>> node_and_size_vec;
            node_and_size_vec.reserve(nodes_list.size() * 4);

            bool finish = false;
            while(!finish)
            {
              int prv_size = nodes_list.size();

                lit = nodes_list.begin();

                int to_expand = 0;

                node_and_size_vec.clear();
                ...
                while(lit != nodes_list.end())
                {
                  ...
                  else
                  {
                      ...
                      if(n1.corners.size() > 0)
                      {
                          nodes_list.push_front(n1);
                          if(n1.corners.size() > 1)
                          {
                              to_expand++;
                              node_and_size_vec.push_back(std::make_pair(
                                        n1.corners.size(), &nodes_list.front()));
                              nodes_list.front().lit = nodes_list.begin();
                          }
                      }
                      if(n2.corners.size() > 0)
                      {
                        nodes_list.push_front(n2);
                          if(n2.corners.size() > 1)
                          {
                              to_expand++;
                              node_and_size_vec.push_back(std::make_pair(
                                      n2.corners.size(), &nodes_list.front()));
                              nodes_list.front().lit = nodes_list.begin();
                          }
                      }
                      if(n3.corners.size() > 0)
                      {
                          ...
                          if(n3.corners.size() > 1)
                          {
                              to_expand++;
                              node_and_size_vec.push_back(std::make_pair(
                                      n3.corners.size(), &nodes_list.front()));
                              nodes_list.front().lit = nodes_list.begin();
                          }
                      }
                      if(n4.corners.size() > 0)
                      {
                          ...
                          if(n4.corners.size() > 1)
                          {
                              to_expand++;
                              node_and_size_vec.push_back(std::make_pair(
                                      n4.corners.size(), &nodes_list.front()));
                              nodes_list.front().lit = nodes_list.begin();
                          }
                      }
                ...
                if((int)nodes_list.size() >= N || (int)nodes_list.size() == prv_size)
                {
                    finish = true;
                }
                else if(((int)nodes_list.size() + to_expand * 3) > N)
                {
                  while(!finish)
                  {
                      prv_size = nodes_list.size();

                      std::vector<std::pair<int, ExtractorNode*>> prv_node_and_size_vec = node_and_size_vec;
                      node_and_size_vec.clear();
                      
                      sort(prv_node_and_size_vec.begin(), prv_node_and_size_vec.end(), CompareNodes);
                      for(int j = prv_node_and_size_vec.size() - 1; j >= 0; --j)
                      {
                          ExtractorNode n1, n2, n3, n4;
                          prv_node_and_size_vec[j].second->DivideNode(n1, n2, n3, n4);
                          ...
                          nodes_list.erase(prv_node_and_size_vec[j].second->lit);

                          if((int)nodes_list.size() >= N)
                              break;
                      }
            ...
            std::vector<Corner> res_corners;
            // res_corners.reserve(nfeatures);
            for(std::list<ExtractorNode>::iterator lit = nodes_list.begin(); lit != nodes_list.end(); lit++)
            {
                std::vector<Corner> &corners = lit->corners;
                Corner* corner = &corners[0];
                float max_score = corner->score;

                for(size_t k = 1; k < corners.size(); ++k)
                {
                    if(corners[k].score > max_score)
                    {
                        corner = &corners[k];
                        max_score = corners[k].score;
                    }
                }

                res_corners.push_back(*corner);
            }
