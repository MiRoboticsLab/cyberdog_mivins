...
namespace mivins
{
    ...
    template <typename T>
    T param(rclcpp::Node::SharedPtr nh, const std::string &name, const T &defaultValue,
            bool silent = false)       
    {
        T v;
        ...
        if (!silent)
        {
            WARN_STREAM("[MIVINS][PARAM]Cannot find value for parameter: " << name << ", assigning default: " << defaultValue);
        }
        return defaultValue;
    }
    ...
    template <typename T>
    T param(nav2_util::LifecycleNode *nh, const std::string &name, const T &defaultValue,
            bool silent = false)     
    {
        T v;
        if (!nh->has_parameter(name))
        ...
        if (!silent)
        {
            WARN_STREAM("[MIVINS][PARAM2]Cannot find value for parameter: " << name << ", assigning default: " << defaultValue);
        }
        return defaultValue;
    }

    MivinsProcess::MivinsProcess(rclcpp::Node::SharedPtr node_nh,
                           const std::string calib_file,
                           const std::string config_file):pnh_(node_nh)
    {
        // : nh_(nh)
        ...
        create_map_client_ = pnh_->create_client<std_srvs::srv::SetBool>("create_map_service");//开始建图
        //finish_map_client_ = pnh_->create_client<std_srvs::srv::SetBool>("finish_map_service");//结束建图
        finish_map_client_ = pnh_->create_client<cyberdog_visions_interfaces::srv::FinishMap>("finish_map_service");//结束建图
        create_nav_client_ = pnh_->create_client<std_srvs::srv::SetBool>("start_nav_service");//开始导航
        stop_nav_client_ = pnh_->create_client<std_srvs::srv::SetBool>("stop_nav_service");//结束导航

        // for following
        control_elevation_map_client_ = pnh_->create_client<std_srvs::srv::SetBool>("elevation_mapping_step_monitor");
    ...
    void MivinsProcess::initMivins() {
        ...
        start_map_ = false;// map
        finish_map_ = false;// map
        reloc_lastreq_time_ = 0;
        b_in_active_server_ = false;
        reloc_num_find_abnormal_ = 3;//reloc 参考帧数设置
        miloc_start_map = false;//map
        miloc_finish_map = false;// map
        end_flag = false;
        mivins_mode_ = LifeCycleMode::DEFAULT;//default
        ...
        const int64_t timestamp_nanoseconds)
    {
        ...
        switch (stage)
        {
            case 2:
            {
                ...
                {
                    INFO_STREAM("[MIVINS][PUBLISHRESULTS]publishResults last_cam_poses.size = " 
                ...
                double time_stamp = (double)timestamp_nanoseconds * 1e-9;
                ...
                Eigen::Quaterniond q_odom(last_odom_poses[0][6],last_odom_poses[0][3],last_odom_poses[0][4],last_odom_poses[0][5]);
                Eigen::Translation3d t_odom(last_odom_poses[0][0],last_odom_poses[0][1],last_odom_poses[0][2]);
                save_twb.precision(16);
                save_twb << time_stamp << " " << last_odom_poses[0][0] << " " << last_odom_poses[0][1] << " " << last_odom_poses[0][2] << " "
                ...
                if(mivins_mode_ == LifeCycleMode::NAVIGATION) {
                    ...
                    std::vector<std::vector<double>> last_imu_pose = MiVins_GetLastFramesPose(sensor_type_imu);
                    ...
                    if(reloc_frame_id_ == 0 &&dtime > reloc_time_interval) {
                        ...
                        auto result = reloc_client_->async_send_request(request,std::bind(&MivinsProcess::RelocCallback, 
                                                                                                    this, 
                                                                                                    std::placeholders::_1));
                        INFO_STREAM("[MIVINS][PUBLISHRESULTS]init reloc request success ------time: "<< std::to_string(reloc_lastreq_time_/1000000000.0));
                        reloc_frame_id_++;
                        last_mivins_pose_ = imu_pose;
                    } 
                    else if(reloc_frame_id_ != 0 &&dtime > reloc_time_interval) {//3s一次请求
                        
                        double distance_pub_reloc_ = sqrt((imu_pose[0] - last_mivins_pose_[0])*(imu_pose[0] - last_mivins_pose_[0]) + 
                        ...
                        if(distance_pub_reloc_> 0.5) {
                            ...
                            auto result = reloc_client_->async_send_request(request,std::bind(&MivinsProcess::RelocCallback, 
                                                                                                        this,
                                                                                                        std::placeholders::_1));
                            INFO_STREAM("[MIVINS][PUBLISHRESULTS]normal reloc request success!!!-- -----time: "<< std::to_string(reloc_lastreq_time_/1000000000.0));
                            reloc_frame_id_++;
                            last_mivins_pose_ = imu_pose;
                        }
                    }
                    ...
                    Eigen::Affine3d trans_map_vodom = visualizer_->publishVodomToMapTF(time_stamp,last_cam_poses[last_cam_poses.size() - 2],
                                                                        relocResponses_,imu_to_camera_calib,baselink_to_imu_calib);
                    if(visualizer_->init_reloc_) 
                    {
                        visualizer_->publishBaselinkToMapOdometry(time_stamp,trans_map_vodom*trans_vodom_baselink,mivins_mode_);//
                        visualizer_->publishBaselinkToVodomTF(time_stamp, trans_vodom_baselink);
                    }
                    /*  ----For Reloc*/
                }
        ...
        const int64_t timestamp_nanoseconds)
    {
        ...
        // INFO_STREAM("[MIVINS][FOLLOWING]publishResults stage = " << stage);//
        switch (stage)
        {
            case 2:
            {
        ...
        const sensor_msgs::msg::Image::ConstSharedPtr msg1)
    {
        ...
        // INFO_STREAM("[MIVINS][STEREOCALLBACK] left timestamp: " << std::fixed << std::setprecision(10) << (double)rclcpp::Time(msg0->header.stamp).nanoseconds() / 1000000.0
        //           << "   right timestamp: " << std::fixed << std::setprecision(10) << (double)rclcpp::Time(msg1->header.stamp).nanoseconds() / 1000000.0
        //           << "   diff time left - right : " << std::fixed << std::setprecision(10) << (double)rclcpp::Time(msg0->header.stamp).nanoseconds() / 1000000.0 - (double)rclcpp::Time(msg1->header.stamp).nanoseconds() / 1000000.0 );
    ...
    void MivinsProcess::stereoProcess()
    {
        ...
        while(true) {
            if(mivins_mode_ == LifeCycleMode::DEFAULT) {
                ...
                if(b_get_start_by_reloc_) {//获取重定位信息跳出
                    break;
                } else {
        ...
        while(!b_reset_)
        {
            ...
            if(normal_mode_ || mivins_mode_ == LifeCycleMode::MAPPING || mivins_mode_ == LifeCycleMode::NAVIGATION)
    ...
    {
        app_create_map_service_ = pnh_->create_service<std_srvs::srv::SetBool>("start_vins_mapping",
                                                                    std::bind(&MivinsProcess::CreateMapCallback, 
                                                                                this, std::placeholders::_1, std::placeholders::_2));
        // app_finish_map_service_ = pnh_->create_service<std_srvs::srv::SetBool>("stop_vins_mapping",
        //                                                             std::bind(&MivinsProcess::FinishMapCallback, 
        //                                                                         this, std::placeholders::_1, std::placeholders::_2));

        app_finish_mapname_service_ = pnh_->create_service<cyberdog_visions_interfaces::srv::FinishMap>("stop_vins_mapping",
                                                                    std::bind(&MivinsProcess::FinishMapCallback, 
                                                                                this, std::placeholders::_1, std::placeholders::_2));
    }
    void MivinsProcess::CreateMapCallback(const std::shared_ptr<std_srvs::srv::SetBool::Request> request,
                                       const std::shared_ptr<std_srvs::srv::SetBool::Response> response)
    {
        if(b_in_active_server_) {
            response->success = true;//false;
            response->message = "app_start_location_service error: please do not duplicate request!!!";
            INFO_STREAM("[MIVINS][CREATEMAPCALLBACK]app_start_mivins_map_service error: please do not duplicate request!!!");
        } else {
    ...
    /*
    void MivinsProcess::FinishMapCallback(const std::shared_ptr<std_srvs::srv::SetBool::Request> request,
                                       const std::shared_ptr<std_srvs::srv::SetBool::Response> response)
    {
        if(!b_in_active_server_) {
            response->success = false;
            response->message = "app_finish_map_service error: please do not duplicate request!!!";
            INFO_STREAM("[MIVINS][CREATEMAPCALLBACK]app_finish_map_service error: please do not duplicate request!!!");
        } else {
            if (request->data) {
                finish_map_ = true;
                response->success = true;
    ...
    */
    ...
    void MivinsProcess::FinishMapCallback(const std::shared_ptr<cyberdog_visions_interfaces::srv::FinishMap::Request> request,
    ...
    {
        app_start_navigation_service_ = pnh_->create_service<std_srvs::srv::SetBool>("start_vins_location",
                                                                    std::bind(&MivinsProcess::StartNavigationCallback, 
                                                                                this, std::placeholders::_1, std::placeholders::_2));
        app_finish_navigation_service_ = pnh_->create_service<std_srvs::srv::SetBool>("stop_vins_location",
                                                                    std::bind(&MivinsProcess::FinishNavigationCallback, 
                                                                                this, std::placeholders::_1, std::placeholders::_2));
    }

    void MivinsProcess::StartNavigationCallback(const std::shared_ptr<std_srvs::srv::SetBool::Request> request,
                                       const std::shared_ptr<std_srvs::srv::SetBool::Response> response)
    {
        if(b_in_active_server_) {
            response->success = true;
            response->message = "app_start_location_service error: please do not duplicate request!!!";
            INFO_STREAM("[MIVINS][CREATENAVIGATIONCALLBACK]app_start_location_service error: please do not duplicate request!!!");
        } else {
    ...
    void MivinsProcess::FinishNavigationCallback(const std::shared_ptr<std_srvs::srv::SetBool::Request> request,
                                       const std::shared_ptr<std_srvs::srv::SetBool::Response> response)
    {
        if(!b_in_active_server_) {
            response->success = true;
            response->message = "app_finish_location_service error: please do not duplicate request!!!";
            INFO_STREAM("[MIVINS][FINISHNAVIGATIONCALLBACK]app_end_navlocation_service error: please do not duplicate request!!!");
        } else {
             if (request->data) {
                response->success = true;
    ...
    void MivinsProcess::deactivateMivins()
    {
        ...
        if (image_thread_ != nullptr) {
            INFO_STREAM("[MIVINS][STOPMIVINS]Mivins_process kill result:   " << pthread_cancel(image_thread_->native_handle()));
    ...
    void MivinsProcess::milocCreatMapCallback(rclcpp::Client<std_srvs::srv::SetBool>::SharedFuture result)
    {
        if(result.get())
        {
    ...
    void MivinsProcess::milocFinishMapCallback(rclcpp::Client<std_srvs::srv::SetBool>::SharedFuture result)
    {
        if(result.get())
        {
    ...
    void MivinsProcess::createWaitRelc() {
        wait_reloc_thread_ = std::unique_ptr<std::thread>(new std::thread(&MivinsProcess::startNavigation, this));
    }
    void MivinsProcess::createStopRelc() {
        stop_reloc_thread_ = std::unique_ptr<std::thread>(new std::thread(&MivinsProcess::stopNavigation, this));
    }
    ...
    void MivinsProcess::startNavigation()
    {
        ...
        while(!create_nav_client_->wait_for_service(1s)) {
            ...
            if (!rclcpp::ok()) {
                ...
                INFO_STREAM("[MIVINS][STARTNAVIGATION]Service startNavigation not available, waiting again..." );
                sleep(1.0);
            }
            n++;
            if(n>5) {
        ...
        request->data = true; //Request: true for visual, false for lidar

        using ServiceResponseFuture =
            rclcpp::Client<std_srvs::srv::SetBool>::SharedFuture;
        auto response_received_callback = [this](ServiceResponseFuture future) {
                bool b_milocStartNav = future.get()->success;
                INFO_STREAM("[MIVINS][STARTNAVIGATION]miloc:If startNavigation success:" << future.get()->success );
                INFO_STREAM("[MIVINS][STARTNAVIGATION]miloc:startNavigation message:" << future.get()->message );
    ...
    void MivinsProcess::startElevationMap()
    {
        ...
        auto future_result = control_elevation_map_client_->async_send_request(request, response_received_callback);
    }
    ...
    void MivinsProcess::finishElevationMap()
    {
        ...
        {
            if (!rclcpp::ok()) 
                INFO_STREAM("[MIVINS][FINISHELEVATIONMAP]Interrupted while waiting for the service. Exiting..." );
            else
                WARN_STREAM("[MIVINS][FINISHELEVATIONMAP]Time out when waiting for elevation mapping service. Skipping..." );
            return;
        }
    ...
    void MivinsProcess::robotStatusCallback(const protocol::msg::MotionStatus::ConstSharedPtr status_msg)
    {
        //start elevation mapping
        // if(mivins_mode_ == LifeCycleMode::FOLLOWING)
        // {
        //     startElevationMap();
        // }
    ...
    void MivinsProcess::subscribeRobotStatus()
    {
        ...
        sub_status = pnh_->create_subscription<protocol::msg::MotionStatus>(
            status_topic, sub_qos, std::bind(&mivins::MivinsProcess::robotStatusCallback, this, std::placeholders::_1)
        );
    }
    ...
    void MivinsProcess::RelocCallback(rclcpp::Client<cyberdog_visions_interfaces::srv::Reloc>::SharedFuture result)
    {
        ...
        INFO_STREAM( "[MIVINS][RELOCCALLBACK]reloc callback reloc_response.time:   " << std::to_string(reloc_response.time));//"   request time: "<<std::to_string(rclcpp::Time(request->header.stamp).nanoseconds()) <<
        ...
        INFO_STREAM("[MIVINS][RELOCCALLBACK]reloc verified: "<< reloc_response.is_verified << " status: "<< reloc_response.status << "  reloc_id: "<<  reloc_response.reloc_id  );
        //if(reloc_response.is_verified || reloc_response.reloc_id == 0)   //因后面是有判断轨迹合理性的
        {
            ...
            if(reloc_response.status == 1 || reloc_response.status == 2) {
                if(reloc_response.confidence >= confidence_threshold ) {
                    ...
                    if(!bRepeat) {
                        ...
                        Eigen::Quaterniond q_imu_to_cam(imu_to_camera_calib[6],imu_to_camera_calib[3],imu_to_camera_calib[4],imu_to_camera_calib[5]);
                        Eigen::Translation3d t_imu_to_cam(imu_to_camera_calib[0],imu_to_camera_calib[1],imu_to_camera_calib[2]);//current frame
                        // reloc_response.z = 0;//TODO:z = 0
                        ...
                        Eigen::Translation3d t_reloc(reloc_response.x,
                                                    reloc_response.y,
                                                    reloc_response.z);//reloc value    T_map_imu   TODO:导航需求z = 0  reloc_response.z
                        for(int posen = visualizer_->path.poses.size() - 1 ; posen >=0  ;posen--) {
                            double odom_time = rclcpp::Time(visualizer_->path.poses[posen].header.stamp).nanoseconds()/1000000000.0;
                            if(abs( odom_time- reloc_time) <reloc_pose_find_time_threshold)
                            {
                                ...
                                Eigen::Translation3d t_vodom(visualizer_->path.poses[posen].pose.position.x,
                                                             ...
                                                             visualizer_->path.poses[posen].pose.position.z);//path存储的左目pose

                                //这里可以添加reloc信息非掉的判断以排除一些异常
                                Eigen::Affine3d uselastrelocinfo_baselink_to_map =  visualizer_->vodom_to_map_*(t_vodom*q_vodom)*(t_imu_to_cam* q_imu_to_cam)*(t_baselink_to_imu*q_baselink_to_imu);
                                
                                //(t_rsleft_to_baselink*q_rsleft_to_baselink).inverse();
                                Eigen::Matrix4d uselastrelocinfo_baselink_to_map_matrix = uselastrelocinfo_baselink_to_map.matrix();
                                ...
                                double distance_reloc = sqrt((reloc_response.x - last_x)*(reloc_response.x - last_x)
                        ...
                        if(b_findpose&&!b_baddata) {
                            ...
                            INFO_STREAM("[MIVINS][RELOCCALLBACK]reloc push into queue sucess!!! "<< relocResponses_.size() );
                            ...
                            visualizer_->publishBaselinkToMapTest(rclcpp::Time(response->pose.header.stamp).nanoseconds()/1000000000.0,trans_test);
                        } else {
                            WARN_STREAM("[MIVINS][RELOCCALLBACK]reloc can not find pose info ,time threshold !" );
                        }
                    }
                    INFO_STREAM("[MIVINS][RELOCCALLBACK]reloc push response  sucess! queue size: "<<relocResponses_.size()  );
                }
            }
    ...
    void MivinsProcess::odomCallback(const nav_msgs::msg::Odometry::ConstSharedPtr odom_msg)
    {
        ...
        if(mivins_mode_ == LifeCycleMode::MAPPING || mivins_mode_ == LifeCycleMode::FOLLOWING) {
            ...
            if(!b_get_start_by_reloc_) {//未接到reloc信息则跳出
        ...
        if(!normal_mode_ && mivins_mode_ == LifeCycleMode::FOLLOWING)
        {
            ...
            // int64_t cur_timestamp_nanoseconds = rclcpp::Clock().now().nanoseconds();
            // if(last_publish_time_ == 0)
            //     last_publish_time_ = cur_timestamp_nanoseconds;
            // if(timestamp_nanoseconds > cur_timestamp_nanoseconds)
            //     timestamp_nanoseconds = last_publish_time_;
            // last_publish_time_ = timestamp_nanoseconds;
    ...
    void MivinsProcess::subscribeImage()
    {
        ...
        INFO_STREAM("[MIVINS][SUBSCRIBEIMAGE]MivinsProcess::subscribeImage() pipeline_type_ = " << pipeline_type_ );
        ...
        else if (pipeline_type_ == 2)
        {
            std::function<void(const sensor_msgs::msg::Image::ConstSharedPtr,
                const sensor_msgs::msg::Image::ConstSharedPtr)> cb = 
                    std::bind(&mivins::MivinsProcess::stereoCallback, this, std::placeholders::_1,
                    std::placeholders::_2);

            std::string cam0_topic  = cam0_topic_;
            std::string cam1_topic = cam1_topic_;
            INFO_STREAM("[MIVINS][SUBSCRIBEIMAGE]-------cam0_topic  " << cam0_topic << "-------cam0_topic  " << cam1_topic );
            auto sub_image0 = std::make_unique<ImageSyncSub>(pnh_, cam0_topic, rclcpp::SensorDataQoS().get_rmw_qos_profile());
            auto sub_image1 = std::make_unique<ImageSyncSub>(pnh_, cam1_topic, rclcpp::SensorDataQoS().get_rmw_qos_profile());
            auto sync = std::make_unique<ImageSync>(ImageSyncPolicy(10), *sub_image0,
                                                    *sub_image1);
