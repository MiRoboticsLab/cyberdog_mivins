  ...
  pc_(new PointCloud)
{
  ...
  pub_baselink_odometry = pnh_->create_publisher<nav_msgs::msg::Odometry>("odom_slam", 1000);
  ...
  pub_reloc_odometry = pnh_->create_publisher<nav_msgs::msg::Odometry>("mivins/reloc_odom", 1000);
  pub_images_.resize(num_cameras);
  pub_cam_poses_.resize(num_cameras);
  for (size_t i = 0; i < num_cameras; ++i) {
    pub_images_.at(i) = pnh_->create_publisher<sensor_msgs::msg::Image>(
      "mivins/image" + std::to_string(
        i), 1000);
    pub_cam_poses_.at(i) = pnh_->create_publisher<geometry_msgs::msg::PoseStamped>(
      "pose_cam/cam" + std::to_string(
        i), 10);
  }
  init_reloc_ = false;
...
void Visualizer::visualizerReset()
{
  ...
  pub_baselink_odometry.reset();
  pub_path.reset();
  pub_odometryleft.reset();
  pub_pathleft.reset();
  pub_odometryright.reset();
  pub_pathright.reset();
  pub_reloc_odometry.reset();
}
...
Eigen::Affine3d Visualizer::publishVodomToMapTF(
  double time_stamp,
  ...
  std::vector<double> baselink_to_imu_calib)
{
  if (relocResponses.size() < 1 || cam_pose.empty()) {
    ...
    Eigen::Quaterniond q_imu_to_cam(imu_to_calib[6], imu_to_calib[3], imu_to_calib[4],
      imu_to_calib[5]);
    Eigen::Translation3d t_imu_to_cam(imu_to_calib[0], imu_to_calib[1], imu_to_calib[2]);

    if (!init_reloc_) {
      INFO_STREAM("[MIVINS][PUBLISHVODOMTOMAPTF]reloc init in ...");
      RelocResponse reloc_response = vreloc[0];
      int posen = 0;

      INFO_STREAM(
        "[MIVINS][PUBLISHVODOMTOMAPTF]reloc timemax : " <<
          std::to_string(
          rclcpp::Time(
            path.poses[posen].header.stamp).nanoseconds() / 1000000000.0) <<
          "reloc timemin : " <<
          std::to_string(rclcpp::Time(path.poses[0].header.stamp).nanoseconds() / 1000000000.0) );
      INFO_STREAM(
        "[MIVINS][PUBLISHVODOMTOMAPTF]reloc path.poses[posen].header.stamp : " <<
          std::to_string(rclcpp::Time(path.poses[posen].header.stamp).nanoseconds()) <<
      ...
      INFO_STREAM(
        "[MIVINS][PUBLISHVODOMTOMAPTF]odom_time: " << std::to_string(
          odom_time) << "  reloc_time: " << std::to_string(reloc_time));
      INFO_STREAM(
        "[MIVINS][PUBLISHVODOMTOMAPTF]reloc getin posetime: " <<
          std::to_string(
          rclcpp::Time(
            path.poses[posen].header.stamp).nanoseconds()) << "   responseTime: " <<
          std::to_string(rclcpp::Time(reloc_response.time).nanoseconds()));
      INFO_STREAM(
        "[MIVINS][PUBLISHVODOMTOMAPTF]verified:" << reloc_response.is_verified <<
        " confidence:" << reloc_response.confidence <<
      ...
      if (reloc_response.confidence >= init_reloc_score_threshold && reloc_response.status == 1) {
        ...
        Eigen::Translation3d t_vodom(path.poses[posen].pose.position.x,
          ...
          path.poses[posen].pose.position.z);
        Eigen::Quaterniond qc_vodom(cam_pose[6], cam_pose[3], cam_pose[4], cam_pose[5]);
        Eigen::Translation3d tc_vodom(cam_pose[0], cam_pose[1], cam_pose[2]);
        vodom_to_map_ = (t_reloc * q_reloc) * (t_baselink_to_imu * q_baselink_to_imu).inverse() *
          (t_imu_to_cam * q_imu_to_cam).inverse() * (t_vodom * q_vodom).inverse();
        last_new_reloc_time_ = vreloc[vreloc.size() - 1].time / 1000000000.0;
        INFO_STREAM(
          "[MIVINS][PUBLISHVODOMTOMAPTF]reloc getin time and the vodom_to_map_ : " <<
            vodom_to_map_.matrix());
        INFO_STREAM("[MIVINS][PUBLISHVODOMTOMAPTF]reloc success get reloc");
        init_reloc_ = true;
        time_vodom_to_map_ = rclcpp::Time(path.poses[posen].header.stamp).nanoseconds();
      } else {
        INFO_STREAM(
          "[MIVINS][PUBLISHVODOMTOMAPTF]confidenceOrStatusFailed!verified:" <<
          reloc_response.is_verified << " id:" << reloc_response.reloc_id);
      }
    } else {
      ...
      if (vreloc.size() < 3) {
        ...
        INFO_STREAM("[MIVINS][PUBLISHVODOMTOMAPTF]reloc : vreloc.size() = 3  yes!");
      }
      ...
      for (int rn = 0; rn < vreloc.size(); rn++) {
        ...
        for (int posen = path.poses.size() - 1; posen >= 0; posen--) {
          double odom_time = rclcpp::Time(path.poses[posen].header.stamp).nanoseconds() /
            1000000000.0;
      ...
      double last_reloc_time = time_vodom_to_map_ / 1000000000.0;
      double reloc_new_time = rclcpp::Time(vreloc[vreloc.size() - 1].time).nanoseconds() /
        1000000000.0;
      double reloc_dt = last_reloc_time - reloc_new_time;
      double reloc_new_time_dt = last_new_reloc_time_ - reloc_new_time;
      if (n_find_index.size() != vreloc.size() || abs(reloc_dt) < 0.5) {
        INFO_STREAM(
          "[MIVINS][PUBLISHVODOMTOMAPTF]find_index.size:" << n_find_index.size() <<
          "!= vreloc.size()" << vreloc.size());
        ...
        for (int rn = 0; rn < vreloc.size(); rn++) {
          ...
          Eigen::Translation3d t_vodom(path.poses[n_find_index[rn]].pose.position.x,
            ...
            path.poses[n_find_index[rn]].pose.position.z);
          Eigen::Affine3d vodom_to_map_temp = (t_reloc * q_reloc) *
            (t_baselink_to_imu * q_baselink_to_imu).inverse() *
            (t_imu_to_cam * q_imu_to_cam).inverse() * (t_vodom * q_vodom).inverse();
          ...
          for (int pn = 0; pn < vreloc.size(); pn++) {
            ...
            Eigen::Translation3d t_vodom(path.poses[n_find_index[pn]].pose.position.x,
              ...
              path.poses[n_find_index[pn]].pose.position.z);
            Eigen::Affine3d imu_to_map_useinit = vodom_to_map_temp * (t_vodom * q_vodom) *
              (t_imu_to_cam * q_imu_to_cam);
            Eigen::Matrix4d imu_to_map_useinit_matrix = imu_to_map_useinit.matrix();
            ...
            double distance_compare = sqrt(
              (reloc_response_compare.x - imu_to_map_useinit_x) *
              (reloc_response_compare.x - imu_to_map_useinit_x) +
              (reloc_response_compare.y - imu_to_map_useinit_y) *
              (reloc_response_compare.y - imu_to_map_useinit_y) +
              (reloc_response_compare.z - imu_to_map_useinit_z) *
              (reloc_response_compare.z - imu_to_map_useinit_z));
            distance_accinitreloc_newrelocvalue += distance_compare;
            INFO_STREAM(
              "[MIVINS][PUBLISHVODOMTOMAPTF]reloc n_time: " <<
                std::to_string(
                rclcpp::Time(
                  reloc_response.time).nanoseconds() / 1000000000.0) << " rn:" << rn <<
          ...
          if (distance_min > distance_accinitreloc_newrelocvalue) {
            ...
            INFO_STREAM(
              "[MIVINS][PUBLISHVODOMTOMAPTF]reloc min_distance:" << distance_min << " select:" <<
                rn);
          }
        }
        if (reloc_n != -1) {
          time_vodom_to_map_ = rclcpp::Time(vreloc[reloc_n].time).nanoseconds();
          ...
          INFO_STREAM(
            "[MIVINS][PUBLISHVODOMTOMAPTF]reloc:min_distance: " << distance_min << " select:" <<
              reloc_n);
          INFO_STREAM(
            "[MIVINS][PUBLISHVODOMTOMAPTF]reloc_x:" << vreloc[reloc_n].x << "reloc_y:" <<
              vreloc[reloc_n].y << "reloc_z:" << vreloc[reloc_n].z);
          INFO_STREAM(
            "[MIVINS][PUBLISHVODOMTOMAPTF]reloc : time : " <<
              std::to_string(rclcpp::Time(vreloc[reloc_n].time).nanoseconds() / 1000000000.0) );
          INFO_STREAM(
            "[MIVINS][PUBLISHVODOMTOMAPTF]reloc verified:" <<
            (int)vreloc[reloc_n].is_verified << " status:" <<
              vreloc[reloc_n].status << " reloc_id: " << vreloc[reloc_n].reloc_id);
          INFO_STREAM(
            "[MIVINS][PUBLISHVODOMTOMAPTF]reloc confidence_threshold:" <<
              vreloc[reloc_n].confidence);
        }
      }
    }
  }
...
void Visualizer::publishVodomToMapTFStatic(double time_stamp)
{
  ...
  geometry_msgs::msg::TransformStamped transformStamped =
    tf2::eigenToTransform(vodom_to_map_static);
  transformStamped.header.stamp = tf2_ros::toMsg(tf2::timeFromSec(time_stamp));
  transformStamped.header.frame_id = "map";
  transformStamped.child_frame_id = "vodom";
  vodommapbr_static.sendTransform(transformStamped);
}
void Visualizer::publishBaselinkToMapOdometry(
  double timestamp_nanoseconds,
  Eigen::Affine3d baselink_to_map, const int & mode)
{
  ...
  Eigen::Translation3d t_baselink(baselink_to_map_matrix(0, 3), baselink_to_map_matrix(
      1,
      3), baselink_to_map_matrix(
      2, 3));
  ...
  odometry.pose.pose.position.z = baselink_to_map_matrix(2, 3);
  if (mode == 2) {
  ...
  const int64_t timestamp_nanoseconds)
{
  ...
  for (size_t i = 0; i < cam_poses.size(); ++i) {
    ...
    odometry.pose.pose.position.z = cam_poses[i][2];
...
Eigen::Affine3d Visualizer::publishBaselinkToVodomTF(
  double time_stamp,
  std::vector<double> cam_pose)
{
  ...
  Eigen::Affine3d trans_odom_baselink = (t_vodom * q_vodom) *
    (t_rsleft_to_baselink * q_rsleft_to_baselink).inverse();
  geometry_msgs::msg::TransformStamped transformStamped =
    tf2::eigenToTransform(trans_odom_baselink);
  transformStamped.header.stamp = tf2_ros::toMsg(tf2::timeFromSec(time_stamp));
...
void Visualizer::publishBaselinkToVodomTF(double time_stamp, Eigen::Affine3d tf_vodom_baselink)
{
  static tf2_ros::TransformBroadcaster br(pnh_);
